...
from gnn_simulator.utilities.inertia_tensors import parallel_axis_offset
...
class CompositeBody(RigidBody):
    ...
    def add_rigid_bodies(self, new_rigid_bodies, quat=None):
        if quat is None:
            quat = self.quat

        self._rigid_bodies.update(new_rigid_bodies)
        ...
        self.I_body = self._compute_inertia_tensor(self.pos, quat)

        self._rigid_bodies_body_vecs = self._compute_body_vecs(self.pos, quat)
        self.body_vecs_tensor = torch.vstack(
            list(self._rigid_bodies_body_vecs.values())
        )
