...
from gnn_simulator.actuation.dc_motor import DCMotor
...
from gnn_simulator.utilities.tensor_utils import tensorify
...
class Spring(BaseStateObject):
    ...
    def compute_force(self,
        ...
        rel_vel_1 = torch.linalg.vecdot(vel_1, spring_pos_vec_unit, dim=1).unsqueeze(1)
        rel_vel_2 = torch.linalg.vecdot(vel_2, spring_pos_vec_unit, dim=1).unsqueeze(1)
...
class ActuatedCable(Cable):
    ...
    def set_rest_length(self, new_rest_length):
        new_rest_length = torch.clamp(new_rest_length, self.min_length, self.max_length)
        self.actuation_length = self._rest_length - new_rest_length
    ...
    def update_rest_length(self,
                           ...
                           dt):
        if self.actuation_length is None:
            self.actuation_length = torch.zeros_like(cable_length)

        dl = self.motor.compute_cable_length_delta(
            control,
            self.winch_r,
            dt
        ).reshape(-1, 1, 1)

        self.set_rest_length(self.rest_length - dl)
