...
from gnn_simulator.gnn_physics.normalizer import AccumulatedNormalizer
...
class BaseRobotGraphDataProcessor(BaseStateObject):
    ...
    def batch_edge_index(self,
        ...
        num_nodes = num_nodes if num_nodes else senders.max() + 1
    ...
    def get_normalize_feats(self, graph: GraphData) -> GraphData:
        """
        ...
        for node_type, feats_dict in self.hier_node_feat_dict.items():
            norm_feats = torch.hstack([
                self.normalizers[k](graph[k])
                # graph[k]
                for k in feats_dict.keys()
            ])
            graph[node_type + '_x'] = self.add_noise(norm_feats)
        ...
        for edge_type, feats_dict in self.hier_edge_feat_dict.items():
            ...
            graph[edge_type + "_edge_attr"] = self.add_noise(norm_feats)

        return graph

    def add_noise(self, x, noise_sig=0.0):
        if self.add_noise_flag and self.training:
            noise = noise_sig * torch.randn_like(x)
            x = x + noise
        return x
    ...
    def batch_state_to_graph(self, states: List[torch.Tensor], **kwargs) -> GraphData:
        ...
        node_pose, prev_node_pose = self.compute_node_poses(states)
    ...
    def compute_node_poses(self, states):
        ...
        node_poses = [last_prev_node_pose]
        for state in states:
        ...
        return node_pose, prev_node_pose
    ...
    def pose2node(self, pose: torch.Tensor, augment_grnd=False) -> torch.Tensor:
        ...
        body_verts = body_verts.repeat(batch_size, 1, 1)
