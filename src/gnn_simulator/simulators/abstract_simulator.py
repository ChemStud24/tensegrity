...
class AbstractPhysicsSimulator(AbstractSimulator):
    """
    ...
    def get_curr_state(self) -> torch.Tensor:
        ...
        :return: Current state tensor
        """
        return self.robot.get_curr_state()
        ...
        for n in iterator:
            if gt_act_lens is not None:
                ...
                for i, cable in enumerate(cables):
                    cable.actuation_length = gt_act_lens[..., i, n].reshape(-1, 1, 1)
                controls = None
            else:
                controls = ctrls[..., n].clone()
            ...
            next_states.append(curr_state.clone())
