...
def sphere_sphere_signed_distance(
    ...
    return point_on_sphere1, point_on_sphere2, signed_distance
...
def sphere_cylinder_signed_distance(
    ...
    unit_vec_sphere = vec_sphere_to_cylinder / (dist_sphere_to_cylinder.unsqueeze(-1) + eps)  # Add eps to avoid div by zero
    ...
    return point_on_sphere, point_on_cylinder, signed_distance
...
def cylinder_cylinder_signed_distance(
    ...
    return point_on_cylinder1, point_on_cylinder2, signed_distance
...
def sphere_static_prism_signed_distance(
    ...
    sign = torch.where(is_inside, -torch.ones_like(dist_center_to_prism), torch.ones_like(dist_center_to_prism))
    ...
    return point_on_sphere, point_on_prism, signed_distance


def cylinder_static_prism_signed_distance(
    cylinder: Cylinder,
    prism: StaticPrism,
    num_axis_samples: int = 20,
    num_circ_samples: int = 20
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    ...
    t_samples = torch.linspace(0.0, 1.0, num_axis_samples, device=end_pt1.device, dtype=end_pt1.dtype)  # (num_axis_samples,)
    
    # Sample angles around circumference
    angles = torch.linspace(0, 2 * torch.pi, num_circ_samples + 1, device=end_pt1.device, dtype=end_pt1.dtype)[:-1]  # (num_circ_samples,)
    ...
    default_vec = torch.tensor([1.0, 0.0, 0.0], device=axis_2d.device, dtype=axis_2d.dtype).unsqueeze(0).expand(batch_size, -1)  # (batch_size, 3)
    ...
    if small_proj_mask.any():
        default_vec2 = torch.tensor([0.0, 1.0, 0.0], device=axis_2d.device, dtype=axis_2d.dtype).unsqueeze(0).expand(batch_size, -1)
    ...
    offset_per_angle = (cos_angles_expanded * u_vec_expanded + sin_angles_expanded * v_vec_expanded) * radius_val  # (batch_size, 3, num_circ_samples)
    ...
    point_on_cylinder_body = torch.einsum("bij,bjk->bik", rot_mat_inv, point_on_cylinder - prism_pos)  # (batch_size, 3, 1)
    ...
    if is_inside.any():
        # For points inside, find the closest face
        min_dist_idx = torch.argmin(torch.abs(dist_to_surface_body), dim=1)
        closest_body_inside_2d = point_on_cylinder_body_2d.clone()
        for i in range(3):
            mask = (min_dist_idx == i) & is_inside.squeeze(-1)
            if mask.any():
                closest_body_inside_2d[mask, i] = torch.sign(point_on_cylinder_body_2d[mask, i]) * half_lens[mask, i]
    ...
    return point_on_cylinder, point_on_prism, signed_distance
...
def sphere_static_rect_plane_signed_distance(
    ...
    # Signed distance: distance from center to plane (with sign based on which side), minus sphere radius
    # Positive distance when on +z_axis side, negative when on -z_axis side
    sign = torch.sign(z_local.squeeze(-1))  # (batch_size, 1)
    signed_distance = sign * dist_center_to_plane - sphere_radius_raw.unsqueeze(-1)  # (batch_size, 1)
    ...
    return point_on_sphere, point_on_plane, signed_distance
