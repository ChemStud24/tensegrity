...
def quat_exp(q: torch.Tensor) -> torch.Tensor:
    ...
    v_norm = torch.clamp_min(v_norm, 1e-6)
...
def quat_ln(q: torch.Tensor) -> torch.Tensor:
    w = q[..., 0:1, :]
    v = q[..., 1:, :]
    ...
    new_v = torch.zeros(v.shape, dtype=v.dtype, device=v.device)
    non_zero_v = torch.where(v_norm != 0)[0]
    new_v[non_zero_v] = v[non_zero_v] * torch.acos(w[non_zero_v]) / v_norm[non_zero_v]

    ln_q = torch.hstack([new_w, new_v])
    return ln_q


def quat_pow(q: torch.Tensor, pow) -> torch.Tensor:
    new_q = pow * quat_ln(q)
    new_q = quat_exp(new_q)
...
def compute_ang_vel_quat(q_prev, q_curr, dt):
    ...
    angle2 = angle - 2 * torch.pi
    angle_flag = angle.abs() > angle2.abs()
    angle[angle_flag] = angle2[angle_flag]
...
def compute_ang_vel_vecs(prev_vec, curr_vec, dt):
    lim = 0.9999999 if prev_vec.dtype == torch.float32 else 0.999999999999999
    ...
    angle = torch.clamp(angle, -lim, lim)
...
def rotate_vec_quat(q, vec):
    ...
    if q.shape[2] == 1 and vec.shape[2] > 1:
        q = q.repeat(1, 1, vec.shape[2])
...
def compute_quat_btwn_z_and_vec(prin_axis):
    prin_axis = prin_axis / prin_axis.norm(dim=1, keepdim=True)
    zeros = torch.zeros(prin_axis[:, 0:1].shape,
                        dtype=prin_axis.dtype,
                        device=prin_axis.device)
