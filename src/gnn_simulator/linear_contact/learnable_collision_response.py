...
from gnn_simulator.state_objects.primitive_shapes import FlatGround
from gnn_simulator.utilities import torch_quaternion
from gnn_simulator.utilities.inertia_tensors import body_to_world
from gnn_simulator.utilities.tensor_utils import zeros


class ContactParameters(BaseStateObject):

    def __init__(self, restitution, baumgarte, friction, friction_damping, rolling_friction=torch.tensor(0.0)):
    ...
    def to(self, device):
        ...
        self.rolling_friction = self.rolling_friction.to(device)

        return self


class DiffCollisionResponseGenerator(BaseStateObject):

    def __init__(self, ground_z=0, sys_precision=torch.float64):
        super().__init__("collision_response_generator")
        self.ground = FlatGround(ground_z)
    ...
    def to(self, device):
        super().to(device)
        self.ground.to(device)
        self.contact_params.to(device)
    ...
    def compute_delta_vel_contact(self, rigid_body1, rigid_body2, next_state1, next_state2,
        ...
        detection_params_t = collision_detector.detect(curr_state1.detach(),
                                                       curr_state2.detach(),
                                                       rigid_body1,
                                                       rigid_body2)
        ...
        for has_collision_t, contact1_t, contact2_t, normal in detection_params_t:
            if has_collision_t.any():
                curr_state1 = torch.concat([curr_state1[:, :7, ...], next_state1[:, 7:, ...].detach()], dim=1)
                curr_state2 = torch.concat([curr_state2[:, :7, ...], next_state2[:, 7:, ...].detach()], dim=1)
        ...
        detection_params_tp = collision_detector.detect(next_state1.detach(),
                                                        next_state2.detach(),
                                                        rigid_body1,
                                                        rigid_body2)
        ...
        for has_collision, contact1_tp, contact2_tp, normal in detection_params_tp:
            ...
            if has_collision_tp.any():
                params = self.compute_contact_params(next_state1.detach(),
                                                     next_state2.detach(),
                                                     rigid_body1,
                                                     rigid_body2,
                                                     has_collision_tp,
    ...
    def baumgarte_contact_impulse(mass_norm, contact1, contact2, baumgarte, delta_t, normal):
        pen_depth = norm(contact2 - contact1, dim=1).unsqueeze(2).detach()
    ...
    def compute_contact_mass(mass1, mass2, inv_inertia1, inv_inertia2, r1, r2, dir_vec):
        mass_inv1 = vecdot(
            cross(matmul(inv_inertia1, cross(r1, dir_vec, dim=1)), r1, dim=1), dir_vec, dim=1
        ).unsqueeze(1)
        mass_inv2 = vecdot(
            cross(matmul(inv_inertia2, cross(r2, dir_vec, dim=1)), r2, dim=1), dir_vec, dim=1
        ).unsqueeze(1)
        mass_contact = torch.clamp_min((1 / mass1 + 1 / mass2 + mass_inv1 + mass_inv2) ** -1, 0.0)
    ...
    def friction_impulse(rel_vel_tan, tangent, impulse_normal, friction_mu, friction_damping, mass_tangent):
        ...
        static_friction = static_friction.detach() * friction_damping
        # friction = (1 - friction_damping) * mass_tangent * rel_vel_tan
        max_friction = friction_mu * norm(impulse_normal.detach(), dim=1, keepdim=True)
        ...
        impulse_tangent = tangent.detach() * friction
