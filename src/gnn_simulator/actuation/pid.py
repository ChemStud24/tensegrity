...
class PID(BaseStateObject):
    ...
    def update_control_by_target_gait(self, current_length, target_gait, rest_length):
        if self.done is None:
            self.done = torch.tensor([False] * target_gait.shape[0],
                                     device=target_gait.device)

        if self.cum_error is None:
            self.cum_error = zeros(target_gait.shape, ref_tensor=target_gait)

        u = zeros(target_gait.shape, ref_tensor=target_gait)
